        ...
        <div className="flex flex-col gap-1.5 md:hidden">
          {divisiList.map((div) => {
            ...
            const absensiHref = hasAbsen
              ? `/admin/absensi?divisi=${encodeURIComponent(div.nama_divisi)}&tanggal=${encodeURIComponent(today.toLocaleDateString("id-ID"))}&openDetail=true`
              : `/admin/absensi?divisi=${encodeURIComponent(div.nama_divisi)}&tanggal=${encodeURIComponent(today.toLocaleDateString("id-ID"))}`

            const laporanHref = (hasLapor || isPartial) && matchedLapor
              ? `/admin/laporan/${matchedLapor.id}`
              : `/admin/laporan?divisi=${encodeURIComponent(div.nama_divisi.includes("Kebersihan") ? "Divisi Kebersihan & Satpam" : div.nama_divisi)}&tanggal=${encodeURIComponent(today.toLocaleDateString("id-ID"))}`
