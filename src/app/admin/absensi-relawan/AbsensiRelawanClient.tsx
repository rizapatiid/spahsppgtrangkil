            ...
            <div className="bg-slate-900 p-4 flex items-center justify-between text-white shrink-0">
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-2">
                  <label className="block text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Periode Cetak</label>
                  {renderPeriodSelectors()}
                </div>
                
                <div>
                  <label className="block text-[11px] font-extrabold text-slate-400 uppercase tracking-wider mb-2">Pilih Divisi</label>
                  <select 
                    value={selectedDivisi} 
                    onChange={(e) => setSelectedDivisi(e.target.value)}
                    className="w-full border border-slate-200 bg-slate-50 p-2.5 rounded-lg text-[13px] font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-100 focus:bg-white focus:border-blue-400 transition-all"
                  >
                    <option value="all">Semua Divisi</option>
                    {divisiList.map(d => <option key={d.id} value={d.id}>{d.nama_divisi}</option>)}
                  </select>
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-2">
                <button type="button" onClick={() => setShowPrintModal(false)} className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-[12px] font-bold transition">Tutup</button>
                <button 
                  type="button" 
