...
with open("src/app/dashboard/riwayat/RiwayatClient.tsx", "r", encoding="utf-8") as f:
    content = f.read()

# 1. Update filteredAbsensi.map variable
old_foto_find = "const foto = fotoAbsensi.find(f => new Date(f.tanggal).getTime() === new Date(absen.tanggal).getTime())"
new_foto_find = "const fotos = fotoAbsensi.filter(f => new Date(f.tanggal).getTime() === new Date(absen.tanggal).getTime())"
content = content.replace(old_foto_find, new_foto_find)

# 2. Update setAbsensiModal
content = content.replace("setAbsensiModal({ absen, foto })", "setAbsensiModal({ absen, fotos })")

# 3. Update list item thumbnail
old_thumb = """                          {/* Thumbnail */}
                          {foto ? (
                            <div 
...
# 4. Update modal header unpacking
content = content.replace("const { absen, foto } = absensiModal", "const { absen, fotos } = absensiModal")

# 5. Update modal body thumbnails
...
with open("src/app/dashboard/riwayat/RiwayatClient.tsx", "w", encoding="utf-8") as f:
    f.write(content)

print("done edit_riwayat.py")
